...
contract BindBox is ERC721 {
    ...
    constructor(address _chainlink) ERC721("bind box", "BBX") {
        _oracle = VRFv2Consumer(_chainlink);
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        return reveal ? string(abi.encodePacked(baseUri, Strings.toString(tokenId),".json")) : blockUrl;
    }
    ...
    function getOracleRandom() public returns(uint){
        uint lastId = _oracle.lastRequestId();
        ...
        if(success){
			...
			(bool success2, uint[] memory r2) = _oracle.getRequestStatus(lastId);
			if(success2){
				t = r2[0];
			}
        }
